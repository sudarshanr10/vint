...
def create_transaction(transaction_in: schemas.TransactionCreate, db: Session = Depends(get_db)):
   user = db.query(models.User).get(1) ##Assuming user_id is 1 for simplicity, later wire this to authenticated user
   if not user:
...
def get_summary(db: Session = Depends(get_db)):
    ...
    results = (
        db.query(
            ...
            .filter(models.Transaction.user_id == 1, models.Transaction.timestamp >= start)
